...
# 分析 okx 数据
@analyse_controller.get("/swap/okx", response_model=SwapDirection, description="AI 分析 okx 永续合约多空信号")
async def analyse_okx(
        symbol: str = Query(..., description="填入合约标识，例如：BTC/USDT:USDT"),
        leverage: int = Query(default=1, description="杠杆倍数，低杠杆倍数信号会更大胆，低杠杆信号会更畏缩一些"),
    ...
    result = await analyse_okx_svc.analyse_compare(
        symbol,
        leverage=leverage,
